...
if ($CaseFilter) { $cases = $cases | Where-Object { $_.Name -like $CaseFilter } }
if (-not $cases -or @($cases).Count -eq 0) { throw "No blind benchmark cases matched." }
...
"@
    $requestObject = [ordered]@{
        ...
        prompt = $prompt
        stream = $false
        options = [ordered]@{ num_ctx = $NumCtx; temperature = $Temperature }
    }
    ...
    try {
        ...
        if ($result.StatusCode -ge 200 -and $result.StatusCode -lt 300) {
            ...
            Write-Utf8File $outFile ($record | ConvertTo-Json -Depth 12)
            Write-Host "OK  $($case.Name) [$($result.StatusCode)]"
        } else {
            $record = [ordered]@{ evaluation="blind-v0.2"; case=$case.Name; model=$Model; num_ctx=$NumCtx; http_status=$result.StatusCode; prompt=$prompt; ollama_error_body=$result.Body }
            Write-Utf8File $outFile ($record | ConvertTo-Json -Depth 12)
            Write-Warning "FAIL $($case.Name) [$($result.StatusCode)]"
            if ($StopOnError) { throw "Ollama returned HTTP $($result.StatusCode) for $($case.Name)" }
        }
    } catch {
